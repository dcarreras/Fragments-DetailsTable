...
export default (components: OBC.Components) => {
  ...
  fragments.memoryOptimized = true;
  fragments.disposalStrategy = "aggressive";
  ...
  const loadFragments = async () => {
    ...
    try {
      ...
      if (!geometryBuffer) {
        alert("No se encontró geometría en el archivo!");
        return;
      }
      ...
      const loadOptions = {
        ...
        relationsMap: undefined,
        memoryOptimized: true,
        chunkSize: 1000000,
        onProgress: (progress: number) => {
          showLoadingBar(progress * 100, 0);
        }
      };
      ...
      if (propsFile) {
        ...
        loadOptions.properties = JSON.parse(json);
      }

      const relationsMapFile = zip.file("relations-map.json");
      if (relationsMapFile) {
        const json = await relationsMapFile.async("string");
        loadOptions.relationsMap = indexer.getRelationsMapFromJSON(json);
      }

      await fragments.load(geometry, loadOptions);
      hideLoadingBar();
      
    } catch (error) {
      console.error("Error al cargar fragmentos:", error);
      alert("Error al cargar el archivo. Por favor, intenta de nuevo.");
      hideLoadingBar();
    }
  };
